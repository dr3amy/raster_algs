...
namespace raster_algs.Task1b
{
    public partial class Task1b : Form
    {
        ...
        void FillWithImage(int x, int y) // точка щелчка мышки
        {
            ...
            {
                if ((x + stepXRight) < fillingImage.Width
                    && !border.Contains(new Point(x + stepXRight, y))
                    ...
                    && fillingImage.GetPixel(x + stepXRight, y) == Color.FromArgb(255, 255, 255, 255))
                {
                    ...
                    //fillingImage.SetPixel(x + stepXRight, y, Color.Aquamarine);
                    stepXRight++;
                }
                else stopRight = true;

                if ((x + stepXLeft) > 0
                    && !border.Contains(new Point(x + stepXLeft, y))
                    ...
                    && fillingImage.GetPixel(x + stepXLeft, y) == Color.FromArgb(255, 255, 255, 255)) // линия по х влево
                {
                    ...
                    //fillingImage.SetPixel(x + stepXLeft, y, Color.Aquamarine);
                    stepXLeft--;
                }
                else stopLeft = true;
            }
